...
# %% auto 0
__all__ = ['app', 'rt', 'create_app', 'index', 'edit', 'backlinkscomp', 'taglinkscomp', 'tags', 'getbacklinks', 'open',
...
def taglinkscomp(tls):
    Lis = []
    if tls:
        Lis = [Li(A(src.title, href=src.url, hx_push_url=src.url, hx_boost=True, id=f"tlink-{uuid.uuid4()}"
                )) for src in tls]
    else:
        Lis = [Li("No links found")]
    # print(Lis)
    return Ul(*Lis)
