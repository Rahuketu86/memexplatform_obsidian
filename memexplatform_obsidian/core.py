...
# %% auto 0
__all__ = ['app', 'rt', 'create_app', 'index', 'edit', 'backlinks', 'open', 'iter_file', 'CollapsibleBlocks', 'embed']
...
@rt
def backlinks(request: Request, session):
    print(request)
    lockey = request.get('current-lockey') or session['lockey']   # current note/page id
    store = DBStore(config) if config.ENABLE_DB_MODE else FileStore(config)
    bls = store.backlinks(lockey)  # should return list of backlink note ids/names
    if bls:
        return (
            Li(A(src.title,
                href=src.url,
                ...
                # hx_swap="innerHTML",
                hx_boost=True,
                id=f"backlink-{uuid.uuid4()}"
            ) for src in bls)
        )
    else: 
        return (Li("No Backlink referring to this document or not supported"))
